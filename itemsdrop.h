...
int get_random_weapon_id() {
    return default_weapons_list[random(0, (int)default_weapons_list.size())].id;
}


int get_random_helmet_id() {
    return default_helmets_list[random(0, (int)default_helmets_list.size())].id;
}


int get_random_chestplate_id() {
    return default_chestplate_list[random(0, (int)default_chestplate_list.size())].id;
}


int get_random_leggings_id() {
    return default_leggings_list[random(0, (int)default_leggings_list.size())].id;
}


int get_random_boots_id() {
    return default_boots_list[random(0, (int)default_boots_list.size())].id;
}


// Legendary Items
int get_random_legendary_weapon_id() {
    return legendary_weapons_list[random(0, (int)legendary_weapons_list.size())].id;
}


int get_random_legendary_helmet_id() {
    return legendary_helmets_list[random(0, (int)legendary_helmets_list.size())].id;
}


int get_random_legendary_chestplate_id() {
    return legendary_chestplate_list[random(0, (int)legendary_chestplate_list.size())].id;
}


int get_random_legendary_leggings_id() {
    return legendary_leggings_list[random(0, (int)legendary_leggings_list.size())].id;
}


int get_random_legendary_boots_id() {
    return legendary_boots_list[random(0, (int)legendary_boots_list.size())].id;
}
...
void dropped_item_print(Player& player, Type& item) {
    ...
    if (is_same<Type, Weapon>::value) {
        weapon_print((Weapon&)item, player, player.get_weapon());
    }
    else {
...
void dropped_item_choose(Player& player, Type& item) {
    while (true) {
        ...
        if (player.trader_save) {
            ...
            ccout("продан", green, true);
        }
        else {
            cout << "Ваш текущий предмет будет  ";
            ccout("утерян", red_font, true);
        }
